...
def LogLikelyhoodFunction1 (read, error_rate): #returns LLH function with one true result
    ...
    LLH_value = StrirlingLogFactorial(read.total_coverage()) - StrirlingLogFactorial(true_variants[0][1]) - \
    - StrirlingLogFactorial(read.total_coverage() - true_variants[0][1]) + true_variants[0][1] * \
    ...
    for first_true in range(0, true_variants[0][1] + 1):
        for second_true in range(0, true_variants[1][1] + 1):
            ...
            if first_err >= 0 and second_err >=0: # checking the correctness
                log1 = StrirlingLogFactorial(assumed_total_for_1st_nycleotyde) - StrirlingLogFactorial(first_true) - StrirlingLogFactorial(first_false) - \
                       StrirlingLogFactorial(first_err) + (first_true) * log(1 - error_rate) + \
                    (first_false) * log(error_rate / 3) + (first_err) * log(2 * error_rate / 3)

                log2 = StrirlingLogFactorial(assumed_total_for_2nd_nycleotyde) \
                       - StrirlingLogFactorial(second_true) - StrirlingLogFactorial(second_false) - \
                       StrirlingLogFactorial(second_err) + (second_true) * log(1 - error_rate) + \
...
def GetBestLLHValue(read, error_rate): # We are given an error rate and have to give the answer - is there one or two maximums and if 2 - what is the share if 2? LLH for 1 is known, for 2 - we have to optimise it by share
    ...
    LLH2 = scipy.optimize.minimize(
        ...
        args=(read, error_rate), method='Nelder-Mead', options = {'xatol': 1e-4})
...
def GetErrorAndSplitReads(data): # returns error rate
    ...
    result = scipy.optimize.minimize(
        ...
        args=(data), method='Nelder-Mead', options= {'xatol': 1e-4}).x
